...
SET 'pipeline.name' = 'finance_p11v2_realtime_state___RUN_ID__';

CREATE TEMPORARY TABLE finance_p11v2_scoring_input (
  run_id STRING,
...
CREATE TEMPORARY TABLE finance_p11v2_risk_output (
  run_id STRING,
  contract_version STRING,
  transaction_id STRING,
  event_time STRING,
...
INSERT INTO finance_p11v2_risk_output
SELECT
  run_id,
  'p11v2_realtime_state_contract_v1' AS contract_version,
  transaction_id,
  ...
  'UPSERT' AS state_operation,
  'hbase_account_risk_state_v1' AS state_store_version,
...
FROM (
  ...
  FROM finance_p11v2_scoring_input
  WHERE run_id = '__RUN_ID__'
    AND contract_version = 'p11v2_realtime_state_contract_v1'
)
WHERE raw_score >= 30;
